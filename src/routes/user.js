...
router.post('/user/singup',async (req, res) =>{
    const { name,email, password, confirm_password, cedula} = req.body;
    const errors = [];
    if(name.length <= 0){
    ...
    if(cedula.length <= 0){
        errors.push({text:'Ingrese la cedula'});
    }
    if(errors.length > 0){
        ...
        const newUser = new User({name, email, password, cedula});
...
router.get('/task/new-task', async(req, res)=>{
    const userList = await User.find();
    res.render('task/new-task',{userList});
